...
(function () {
  ...
  var TITLE = 'ЗАЯВЛЕНИЕ', TITLE_CYR = 'ЗАЯВЛЕНИЕ';
  ...
  function renderForm() {
    ...
    if (isMobile()) {
      ...
      elForm.innerHTML = '<div class="step-title">' + (state.qadam + 1) + '-й шаг: ' + q.nom + '</div>' + q.maydonlar.map(fieldHtml).join('');
  ...
  function buildParts() {
    ...
    data.manzil_blok = get('manzil') ? 'проживающего по адресу: ' + get('manzil') + ', ' : '';
    data.telefon_blok = get('telefon') ? 'Тел: ' + get('telefon') + '\n' : '';
    data.sana_blok = get('sana') ? 'с ' + get('sana') : '';
    data.kurs_blok = get('kurs') ? ', ' + get('kurs') : '';
  ...
  function makeBlankDocx() {
    ...
    var xml = '';
    xml += para('(Адрес)' + line, { indent: SHAPKA_INDENT, jc: 'right', after: 60 });
    xml += para('(Кому)__________________', { indent: SHAPKA_INDENT, jc: 'right', after: 60 });
    xml += para('(От кого)______________', { indent: SHAPKA_INDENT, jc: 'right', after: 80 });
    xml += para('', { after: 80 });
    xml += para(TITLE, { jc: 'center', b: true, after: 220 });
    xml += para('Прошу Вас ________________________________________________', { jc: 'both', after: 60 });
    xml += para('______________________________________________________', { after: 60 });
    xml += para('________________________________________________.', { after: 200 });
    xml += footerPara('(Ф.И.О. заявителя)', '(Дата)');
    return docxWrap(xml);
  }

  /* ── Yuklab olish ── */
  function fname(ext, blank) {
    return 'zayavlenie-' + (blank ? 'blank' : tur().id) + '-toolbase.uz.' + ext;
  }
  ...
  $$('[data-act="docx"]').forEach(function (b) { b.addEventListener('click', function () { makeDocx().then(function (bl) { dl(bl, fname('docx')); toast('Word скачан ✓'); }); }); });
  $$('[data-act="blank"]').forEach(function (b) { b.addEventListener('click', function () { makeBlankDocx().then(function (bl) { dl(bl, fname('docx', true)); toast('Бланк скачан ✓'); }); }); });
  $$('[data-act="txt"]').forEach(function (b) { b.addEventListener('click', function () { dl(new Blob([buildText()], { type: 'text/plain;charset=utf-8' }), fname('txt')); toast('Текст скачан ✓'); }); });
  $$('[data-act="copy"]').forEach(function (b) { b.addEventListener('click', function () { navigator.clipboard.writeText(buildText()).then(function () { toast('Скопировано ✓'); }); }); });
